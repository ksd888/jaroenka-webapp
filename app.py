...
for p in selected:
    ...
    with col1:
        st.button("➖", key=f"dec_{safe_key(p)}", on_click=lambda p=p: st.session_state.quantities.__setitem__(p, max(1, st.session_state.quantities[p] - 1)))
    with col2:
        st.markdown(f"<div style='text-align:center; font-size:24px'>{qty}</div>", unsafe_allow_html=True)
    with col3:
        st.button("➕", key=f"inc_{safe_key(p)}", on_click=lambda p=p: st.session_state.quantities.__setitem__(p, st.session_state.quantities[p] + 1))
