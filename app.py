...
if st.button("✅ ยืนยันการขาย"):
    ...
    for item, qty in st.session_state.cart:
        ...
        row = row.iloc[0]
        try:
            worksheet.update_cell(index + 2, df.columns.get_loc("ออก") + 1, safe_safe_int(row["ออก"]) + qty)
            worksheet.update_cell(index + 2, df.columns.get_loc("คงเหลือในตู้") + 1, safe_safe_int(row["คงเหลือในตู้"]) - qty)
