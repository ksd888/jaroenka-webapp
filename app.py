...
with st.expander("📦 เติมสินค้า"):
    ...
    if st.button("📥 ยืนยันเติมสินค้า"):
        ...
        worksheet.update_cell(row_idx, df.columns.get_loc("เข้า")+1, new_in)
        worksheet.update_cell(row_idx, df.columns.get_loc("คงเหลือในตู้")+1, new_left)
...
with st.expander("✏️ แก้ไขสินค้า"):
    ...
    if st.button("💾 บันทึกการแก้ไข"):
        worksheet.update_cell(row_idx, df.columns.get_loc("ราคาขาย")+1, new_price)
        worksheet.update_cell(row_idx, df.columns.get_loc("ต้นทุน")+1, new_cost)
        worksheet.update_cell(row_idx, df.columns.get_loc("คงเหลือในตู้")+1, new_stock)
