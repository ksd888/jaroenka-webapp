...
# === ตั้งค่าระบบ Google Sheets ===
scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
credentials = Credentials.from_service_account_info(st.secrets["GCP_SERVICE_ACCOUNT"], scopes=scope)
gc = gspread.authorize(credentials)
spreadsheet = gc.open_by_key("1HVA9mDcDmyxfKvxQd4V5ZkWh4niq33PwVGY6gwoKnAE")
sheet_ice = spreadsheet.worksheet("iceflow")
sheet_sales = spreadsheet.worksheet("ยอดขาย")

# === วันที่วันนี้ ===
today = datetime.datetime.now(timezone("Asia/Bangkok")).strftime("%Y-%m-%d")

# === โหลดข้อมูลล่าสุด ===
df_ice = pd.DataFrame(sheet_ice.get_all_records())
if df_ice.empty or df_ice.iloc[0]["วันที่"] != today:
    sheet_ice.update("A2", [["", "", "", "", "", "", "", ""]]*3)  # รีเซ็ตยอดเก่า

# === UI เริ่มต้น ===
st.title("🧊 ระบบขายน้ำแข็งเจริญค้า")
st.subheader("📦 โซนรับเข้าน้ำแข็ง")
col1, col2, col3 = st.columns(3)
with col1:
    mo_in = st.number_input("📥 น้ำแข็งโม่ (ถุง)", min_value=0, step=1)
with col2:
    big_in = st.number_input("📥 หลอดใหญ่ (ถุง)", min_value=0, step=1)
with col3:
    small_in = st.number_input("📥 หลอดเล็ก (ถุง)", min_value=0, step=1)

st.divider()
st.subheader("💸 โซนขายน้ำแข็ง")
col4, col5, col6 = st.columns(3)
with col4:
    mo_out = st.number_input("💸 น้ำแข็งโม่ ขายออก", min_value=0, step=1)
with col5:
    big_out = st.number_input("💸 หลอดใหญ่ ขายออก", min_value=0, step=1)
with col6:
    small_out = st.number_input("💸 หลอดเล็ก ขายออก", min_value=0, step=1)

st.divider()
if st.button("✅ ยืนยันการขายน้ำแข็ง"):
    # คำนวณราคาขายและต้นทุน
    prices = {"โม่": 30, "หลอดใหญ่": 55, "หลอดเล็ก": 60}
    costs = {"โม่": 18, "หลอดใหญ่": 35, "หลอดเล็ก": 35}
    summary = []

    for name, out, cost, price in zip(["โม่", "หลอดใหญ่", "หลอดเล็ก"],
                                       [mo_out, big_out, small_out],
                                       [costs["โม่"], costs["หลอดใหญ่"], costs["หลอดเล็ก"]],
                                       [prices["โม่"], prices["หลอดใหญ่"], prices["หลอดเล็ก"]]):
        if out > 0:
            profit = (price - cost) * out
            sheet_sales.append_row([today, name, out, price, cost, out*price, profit])
            summary.append((name, out*price, profit))

    # บันทึกลง iceflow
    sheet_ice.update("A2", [[today, mo_in, mo_out, prices["โม่"], costs["โม่"],
                             big_in, big_out, prices["หลอดใหญ่"], costs["หลอดใหญ่"],
                             small_in, small_out, prices["หลอดเล็ก"], costs["หลอดเล็ก"]]])

    st.success("✅ บันทึกยอดขายน้ำแข็งเรียบร้อยแล้ว")
    for s in summary:
        st.info(f"{s[0]}: ยอดขาย {s[1]} บาท / กำไร {s[2]} บาท")
