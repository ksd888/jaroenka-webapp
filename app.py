...
.stButton>button {
    color: white !important; background-color: #007aff !important; border: none; border-radius: 10px;
...
.st-expander, .st-expander>details {
    background-color: #f9f9f9 !important; color: #000000 !important; border-radius: 8px;
}
...
# 📥 เลือกสินค้า
for p in st.session_state.get("search_items", []):
...
if st.session_state.cart:
    ...
    st.number_input("💰 รับเงิน", key="paid_input", step=1.0, value=st.session_state.paid_input)

    # 🔘 ปุ่มยอดเงินด่วน
    col20, col50, col100, col500, col1000 = st.columns(5)
    with col20: if st.button("20"): st.session_state.paid_input += 20
    with col50: if st.button("50"): st.session_state.paid_input += 50
    with col100: if st.button("100"): st.session_state.paid_input += 100
    with col500: if st.button("500"): st.session_state.paid_input += 500
    with col1000: if st.button("1000"): st.session_state.paid_input += 1000
    ...
    if st.button("✅ ยืนยันการขาย"):
        ...
        for item, qty in st.session_state.cart:
            ...
            worksheet.update_cell(ws_idx, df.columns.get_loc("ออก")+1, new_out)
            worksheet.update_cell(ws_idx, df.columns.get_loc("คงเหลือในตู้")+1, new_left)
...
with st.expander("📦 เติมสินค้า"):
    restock_item = st.selectbox("เลือกสินค้า", product_names)
    restock_qty = st.number_input("จำนวนที่เติม", min_value=1, step=1)
    if st.button("📥 ยืนยันเติมสินค้า"):
        idx = df[df["ชื่อสินค้า"] == restock_item].index[0]
        row = df.loc[idx]
        new_in = safe_safe_int(row["เข้า"]) + restock_qty
        new_left = safe_safe_int(row["คงเหลือในตู้"]) + restock_qty
        worksheet.update_cell(idx+2, df.columns.get_loc("เข้า")+1, new_in)
        worksheet.update_cell(idx+2, df.columns.get_loc("คงเหลือในตู้")+1, new_left)
        st.success(f"✅ เติม {restock_item} แล้ว")

# ✏️ แก้สินค้า
with st.expander("✏️ แก้ไขสินค้า"):
    edit_item = st.selectbox("เลือกรายการ", product_names)
    idx = df[df["ชื่อสินค้า"] == edit_item].index[0]
    row = df.loc[idx]
    new_price = st.number_input("ราคาขาย", value=safe_safe_float(row["ราคาขาย"]))
    new_cost = st.number_input("ต้นทุน", value=safe_safe_float(row["ต้นทุน"]))
    new_stock = st.number_input("คงเหลือในตู้", value=safe_safe_int(row["คงเหลือในตู้"]), step=1)
    if st.button("💾 บันทึกการแก้ไข"):
        worksheet.update_cell(idx+2, df.columns.get_loc("ราคาขาย")+1, new_price)
        worksheet.update_cell(idx+2, df.columns.get_loc("ต้นทุน")+1, new_cost)
        worksheet.update_cell(idx+2, df.columns.get_loc("คงเหลือในตู้")+1, new_stock)
