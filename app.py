...
if st.button("✅ ยืนยันการขาย"):
    ...
    for item, qty in st.session_state.cart:
        ...
        try:
            worksheet.update_cell(idx_in_sheet, df.columns.get_loc("ออก") + 1, safe_safe_int(row["ออก"]) + qty)
            worksheet.update_cell(idx_in_sheet, df.columns.get_loc("คงเหลือในตู้") + 1, safe_safe_int(row["คงเหลือในตู้"]) - qty)
...
with st.expander("📦 เติมสินค้า"):
    ...
    row = df[df["ชื่อสินค้า"] == restock_item]
    if not row.empty:
        index = row.index[0]
        row = row.iloc[0]
        if st.button("📥 ยืนยันเติมสินค้า"):
            try:
                ...
                worksheet.update_cell(index + 2, df.columns.get_loc("เข้า") + 1, new_in)
                worksheet.update_cell(index + 2, df.columns.get_loc("คงเหลือในตู้") + 1, new_left)
                st.success(f"✅ เติม {restock_item} แล้ว")
            except Exception as e:
                st.error(f"❌ อัปเดตไม่สำเร็จ: {e}")
    else:
        st.error("❌ ไม่พบสินค้าใน Google Sheet")

# ✏️ แก้ไขสินค้า
with st.expander("✏️ แก้ไขสินค้า"):
    edit_item = st.selectbox("เลือกรายการ", product_names, key="edit_select")
    row = df[df["ชื่อสินค้า"] == edit_item]
    if not row.empty:
        index = row.index[0]
        row = row.iloc[0]
        new_price = st.number_input("ราคาขายใหม่", value=safe_safe_float(row["ราคาขาย"]), step=1.0)
        new_cost = st.number_input("ต้นทุนใหม่", value=safe_safe_float(row["ต้นทุน"]), step=1.0)
        new_stock = st.number_input("คงเหลือใหม่", value=safe_safe_int(row["คงเหลือในตู้"]), step=1)
        if st.button("💾 บันทึกการแก้ไข"):
            try:
                worksheet.update_cell(index + 2, df.columns.get_loc("ราคาขาย") + 1, new_price)
                worksheet.update_cell(index + 2, df.columns.get_loc("ต้นทุน") + 1, new_cost)
                worksheet.update_cell(index + 2, df.columns.get_loc("คงเหลือในตู้") + 1, new_stock)
                st.success(f"✅ อัปเดต {edit_item} แล้ว")
            except Exception as e:
                st.error(f"❌ ไม่สามารถอัปเดต Google Sheet: {e}")
    else:
        st.error("❌ ไม่พบสินค้าใน Google Sheet")
...
if st.button("🔁 รีเซ็ตยอดเข้า-ออก (เริ่มวันใหม่)", key="reset_io"):
    try:
        ...
        st.error(f"❌ ไม่สามารถรีเซ็ตข้อมูล: {e}")
