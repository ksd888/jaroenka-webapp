...
if st.session_state.cart:
    ...
    st.session_state.paid_input = st.number_input("💰 รับเงิน", value=st.session_state.paid_input, step=1.0, key="paid_input")

    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        if st.button("20", key="money_20"):
            st.session_state.paid_input += 20
            st.experimental_rerun()
    with col2:
        if st.button("50", key="money_50"):
            st.session_state.paid_input += 50
            st.experimental_rerun()
    with col3:
        if st.button("100", key="money_100"):
            st.session_state.paid_input += 100
            st.experimental_rerun()
    with col4:
        if st.button("500", key="money_500"):
            st.session_state.paid_input += 500
            st.experimental_rerun()
    with col5:
        if st.button("1000", key="money_1000"):
