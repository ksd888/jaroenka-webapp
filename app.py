...
def get_current_time():
    return datetime.datetime.now(timezone("Asia/Bangkok")).strftime("%Y-%m-%d %H:%M:%S")
...
def main():
    ...
    if st.session_state.page == "Dashboard":
        ...
        try:
            ...
            st.markdown(f'<div class="custom-success">วันที่ {top_day} มียอดขายสูงสุด {top_sales:,.0f} บาท</div>', unsafe_allow_html=True)
    ...
    # ==================== PRODUCT SALES PAGE ====================
        ...
        col1, col2, col3 = st.columns(3)
        with col1: st.button("20", on_click=lambda: st.session_state.update({"paid_input": st.session_state.paid_input + 20, "last_paid_click": 20}))
        with col2: st.button("50", on_click=lambda: st.session_state.update({"paid_input": st.session_state.paid_input + 50, "last_paid_click": 50}))
        with col3: st.button("100", on_click=lambda: st.session_state.update({"paid_input": st.session_state.paid_input + 100, "last_paid_click": 100}))
        col4, col5 = st.columns(2)
        with col4: st.button("500", on_click=lambda: st.session_state.update({"paid_input": st.session_state.paid_input + 500, "last_paid_click": 500}))
        with col5: st.button("1000", on_click=lambda: st.session_state.update({"paid_input": st.session_state.paid_input + 1000, "last_paid_click": 1000}))
    ...
    # ==================== ICE SALES PAGE ====================
        ...
        try:
            ...
            st.error(f"เกิดข้อผิดพลาดในการโหลดหน้าขายน้ำแข็ง: {str(e)}")
