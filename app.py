...
def main():
    try:
        ...
        # ตรวจสอบการเชื่อมต่ออีกครั้ง (optional)
        gc = connect_google_sheets()
        if not gc:
            st.error("การเชื่อมต่อกับ Google Sheets ถูกตัด กรุณารีเฟรชหน้า")
            st.stop()
        ...
        with col2:
            if st.button("📋 คัดลอกข้อผิดพลาด", key="copy_error"):
                if PYPERCLIP_AVAILABLE:  # ใช้ตัวแปรที่ประกาศไว้ตอนต้นไฟล์
    ...
    try:
        ...
        gc = connect_google_sheets()
        if not gc:
            st.error("ไม่สามารถเชื่อมต่อกับ Google Sheets ได้ กรุณาตรวจสอบการเชื่อมต่ออินเทอร์เน็ตและสิทธิ์การเข้าถึง")
            st.stop()
            
        main()
    except Exception as e:
        logger.error(f"App initialization failed: {e}", exc_info=True)
        st.error("เกิดข้อผิดพลาดในการเริ่มต้นแอปพลิเคชัน")
